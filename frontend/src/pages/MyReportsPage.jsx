...
export default function MyReportsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      // Enrich room title for better UX
      const ids = (res.items || []).map((r) => r.roomId).filter(Boolean).slice(0, 20);
      ...
      await Promise.all(
        uniqueIds.map(async (id) => {
          try {
            ...
            newMap[id] = room?.title || null;
          } catch {
            newMap[id] = null;
          }
        })
      );
      setRoomMap((prev) => ({ ...prev, ...newMap }));
    } catch (e) {
  ...
  return (
    <div className="grid" style={{ gap: 14 }}>
      <div className="card">
        <h2 style={{ marginTop: 0 }}>Báo cáo của tôi</h2>
      ...
      <div className="card">
        {loading ? (
          ...
          <div style={{ color: 'rgba(232,238,252,0.7)' }}>Chưa có báo cáo nào.</div>
        ) : (
            ...
            {items.map((r) => (
              ...
              >
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span className="pill">Trạng thái: {r.status}</span>
                </div>
                <div style={{ color: 'rgba(232,238,252,0.8)' }}>{r.reason}</div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  <Link className="btn" to={`/rooms/${r.roomId}`}>
                    Xem phòng
                  </Link>
                </div>
              </div>
